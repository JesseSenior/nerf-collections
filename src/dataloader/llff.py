...
def load_llff_data(
    ...
    **kwargs,
    ...
    print("[LLFF] Loaded", basedir, bds.min(), bds.max())
    ...
    if spherify:
        ...
        c2w = poses_avg(poses)
        print("[LLFF] recentered", c2w.shape)
        print(c2w[:3, :4])
    ...
    print("[LLFF] HOLDOUT view is", i_test)
    ...
    hwf = poses[0, :3, -1]
    poses = poses[:, :3, :4]
    print("[LLFF] Loaded llff", images.shape, render_poses.shape, hwf, basedir)
    if not isinstance(i_test, list):
        i_test = [i_test]

    if llffhold > 0:
        print("[LLFF] Auto LLFF holdout,", llffhold)
    ...
    print(f"[LLFF] NEAR: {near:.5f} FAR: {far:.5f}")
